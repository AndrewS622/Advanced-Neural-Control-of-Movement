...
% that contains the labels and the number of images having each label. The datastore 
...
% network as the |OutputSize| argument.
...
for nlisti = 1:NList
    numTrainFiles = numTrainFiles_list(nlisti);
...
% |filterSize|, which is the height and width of the filters the training function 
% uses while scanning along the images. In this example, the number 3 indicates 
% that the filter size is 3-by-3. You can specify different sizes for the height 
% and width of the filter. The second argument is the number of filters, |numFilters|, 
% which is the number of neurons that connect to the same region of the input. 
% This parameter determines the number of feature maps. Use the |'Padding'| name-value 
...
% the sensitivity to network initialization. Use <docid:nnet_ref.mw_b7913af4-3a40-4020-bb2c-18c946f5eadd 
...
% sometimes followed by a down-sampling operation that reduces the spatial size 
% of the feature map and removes redundant spatial information. Down-sampling 
% makes it possible to increase the number of filters in deeper convolutional 
% layers without increasing the required amount of computation per layer. One 
% way of down-sampling is using a max pooling, which you create using  <docid:nnet_ref.mw_d2785483-a560-4276-a1c0-daa5f58a1d4b 
...
% followed by one or more fully connected layers. As its name suggests, a fully 
% connected layer is a layer in which the neurons connect to all the neurons in 
% the preceding layer. This layer combines all the features learned by the previous 
% layers across the image to identify the larger patterns. The last fully connected 
% layer combines the features to classify the images. Therefore, the |OutputSize| 
% parameter in the last fully connected layer is equal to the number of classes 
% in the target data. In this example, the output size is 10, corresponding to 
% the 10 classes. Use <docid:nnet_ref.mw_1e7fbc56-4746-4f30-8cd9-7048ce806a0d 
% fullyConnectedLayer> to create a fully connected layer.
% 
% *Softmax Layer* The softmax activation function normalizes the output of 
% the fully connected layer. The output of the softmax layer consists of positive 
...
% layer uses the probabilities returned by the softmax activation function for 
% each input to assign the input to one of the mutually exclusive classes and 
% compute the loss. To create a classification layer, use <docid:nnet_ref.bu5lho8 
% classificationLayer>.
%% Specify Training Options
% After defining the network structure, specify the training options. Train 
% the network using stochastic gradient descent with momentum (SGDM) with an initial 
% learning rate of 0.01. Set the maximum number of epochs to 4. An epoch is a 
% full training cycle on the entire training data set. Monitor the network accuracy 
...
% calculates the accuracy on the validation data at regular intervals during training. 
% The validation data is not used to update the network weights. Turn on the training 
...
% is available (requires Parallel Computing Toolbox™ and a CUDA® enabled GPU with 
% compute capability 3.0 or higher). Otherwise, it uses a CPU. You can also specify 
% the execution environment by using the |'ExecutionEnvironment'| name-value pair 
...
% plot, see <docid:nnet_examples.mw_507458b6-14c3-4a31-884c-9f2119ff7e05 Monitor 
% Deep Learning Training Progress>. The loss is the cross-entropy loss. The accuracy 
% is the percentage of images that the network classifies correctly.
...
% predicts correctly. In this case, more than 99% of the predicted labels match 
...
figure;
semilogy(numTrainFiles_list,1-accuracy); title('Error rate as a function of training set size');
